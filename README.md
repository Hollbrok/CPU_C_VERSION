...
# CPU_C_VERSION
__C__ (with some __C++__ shit) version of __CPU__ with registers, labels, jump ,conditional jumps, functions.
...
- [ ] _Assembler_
    - [X] Обработка первоначального текста.
        ...
        - [X] Обработка меток с самого начала
            - [X] Заполнения массива структур меток в самом начале, для более быстрого ассемблирования кода.
...
### TODO  
* Нужно сразу пройтись по всем командам и распознать метки, а не каждый раз при виде меток проходиться по всем командам (первый вариант НАМНОГО эффективней)!!!
* __make_all__.
